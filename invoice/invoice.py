...
class InVoice(commands.Cog):
    ...
    async def on_guild_channel_create(self, vc):
        ...
        embed1 = discord.Embed(title="__**Voice Chat Commands**__", description="Welcome to your very own text channel! This channel has been created alongside your voice channel for any pictures or links that you may want to share or bot commands that you wish to run.\nTalking of Bot commands, here are some sample commands for using the bots on this server. All commands must be written in this channel. Any problems, mention <@&610429764875845663> \n\n__**Zen Chat Bot**__\nZen Chat dynamically renames the current voice channel according to the game you are playing e.g.「Overwatch」and will rename itself if you start playing a different game. Voice channel name updates occur **upto** every 10 minutes.", color=0x4FE0E0)
