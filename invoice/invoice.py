...
class InVoice(commands.Cog):
    ...
    async def on_guild_channel_create(self, vc):
        ...
        embed1 = discord.Embed(title="__**Voice Chat Commands**__", description="Welcome to your very own text channel!\nThis channel has been created alongside your voice channel for any pictures or links that you may want to share or bot commands that you wish to run.\nTalking of Bot commands, below are some sample commands for using the bots on this server. All commands must be written in this channel. Any problems, mention <@&610429764875845663> \n\n__**Zen Chat Bot**__\nZen Chat dynamically renames the current voice channel according to the game you are playing e.g.「Overwatch」and will rename itself if you start playing a different game. Voice channel name updates occur **upto** every 10 minutes.", color=0x4FE0E0)
        ...
        embed1.add_field(name="`zc name <FINAL FANTASY XIV>`", value="Run this command to change the name of your voice channel manually. Only the channel creator can do this. **Can take up to 10 minutes to update.**", inline=True)
        embed1.add_field(name="`zc bitrate <64>`", value="Set a server-wide custom bitrate (in kbps) for yourself that will be used for any channels you join. By default, all channels are 128kbps, use 64 or 32 if you have slow internet.", inline=True)
        embed1.add_field(name="`zc help <Command>`", value="Use zchelp <command> to get more information about a particular command e.g. `zc help name`", inline=True)
        await channel.send(embed=embed1)
        embed2 = discord.Embed(title="__**Voice Chat Commands**__", description="__**DJ Lucio Bot**__\nDJ Lucio is a feature rich, easy to use Discord music bot. Supporting many sources of music including YouTube, SoundCloud, Twitch and more. See below for a short list of commands:", color=0x4FE0E0)
        ...
        embed2.add_field(name="`DJ play`", value="Plays a song with the given name or url", inline=True)
        embed2.add_field(name="`DJ np`", value="Shows what song the bot is currently playing.", inline=True)
        embed2.add_field(name="`DJ skip`", value="Skips the currently playing song.", inline=True)
        embed2.add_field(name="`DJ loop`", value="Loop the currently playing song.", inline=True)
        embed2.add_field(name="`DJ pause`", value="Pauses the currently playing track.", inline=True)
        embed2.add_field(name="`DJ resume`", value="Resume paused music.", inline=True)
        embed2.add_field(name="`DJ lyrics`", value="Gets the lyrics of the current playing song.", inline=True)
        embed2.add_field(name="`DJ clear`", value="Clears the queue.", inline=True)
        embed2.add_field(name="`DJ help`", value="Shows a list of **all** commands.", inline=True)
